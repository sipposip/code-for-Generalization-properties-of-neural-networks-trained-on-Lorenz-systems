...
for selection_strategy in ('density', 'density-full'):
    ...
    plt.savefig(f'{plotdir}/model_full.png')

    lorenz3dplot_lines(network_clim_full, 'network full clim', alpha=0.8)
    plt.savefig(f'{plotdir}/network_clim_full.png')
    ...
    for lims in all_lims:
        ...
        # the error
        pred_full = network_full.predict(X_test_full)
        pred_trunc = network_truncated.predict(X_test_trunc)
        ...
        pred_trunc_on_full = network_truncated.predict(X_test_full)
        ...
        for _ in range(N_fc):
            current = network_truncated.predict(current)
